...
int verifierEmplacementTuile(struct Tuile*** grille, struct Tuile* tuile, int x, int y){
    ...
    */
    ...
    //haut
    if(x>0 && grille[y-1][x] != NULL && !batimentsEgaux(grille[y-1][x]->elements[2],tuile->elements[0])){
        return 0;
    }
    //bas
    if(x<taille-1 && grille[y+1][x] != NULL && !batimentsEgaux(grille[y+1][x]->elements[0],tuile->elements[2])){
        return 0;
    }
    //gauche
    if(y>0 && grille[y][x-1] != NULL && !batimentsEgaux(grille[y][x-1]->elements[1],tuile->elements[3])){
        return 0;
    }
    //droite
